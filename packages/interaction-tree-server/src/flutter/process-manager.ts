...
class FlutterProcessManager extends EventEmitter {
  ...
  private state: FlutterAppState | null = null;
   ...
   * Run a Flutter app.
   */
  async run(options: RunOptions): Promise<FlutterAppState> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleOutput = (data: Buffer, isStderr: boolean) => {
        ...
        for (const line of lines) {
          ...
          if (vmServiceUri && !resolved && (
            FLUTTER_RUN_READY_REGEX.test(line) || 
   ...
   */
  async rebuild(options: RebuildOptions): Promise<FlutterAppState> {
  ...
  private async connectToApp(uri: string): Promise<void> {
  ...
  private handleFlutterEvent(event: { event?: string; params?: Record<string, unknown> }): void {
    switch (event.event) {
      case 'app.started':
...
// Singleton instance
let manager: FlutterProcessManager | null = null;

export function getFlutterProcessManager(): FlutterProcessManager {
  if (!manager) {
