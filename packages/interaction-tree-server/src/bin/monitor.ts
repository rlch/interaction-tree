...
function formatEvent(event: MonitoringEvent): string {
...
function shouldInclude(event: MonitoringEvent): boolean {
...
ws.on('message', (data) => {
  try {
    const event = JSON.parse(data.toString()) as MonitoringEvent;
